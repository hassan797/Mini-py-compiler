...
class Ebinop extends Expr {
  ...
  @Override
  public String toString() {
    return "Ebinop [op=" + op + ", e1=" + e1 + ", e2=" + e2 + "]";
  }
}
...
class Ecall extends Expr {
  final Ident f;                // called function name
  final LinkedList<Expr> argspassed;    // arguments passed
  ...
  @Override
  public String toString(){
    String s = "Identifier : "+ f.id+  "\n"+ 
                "Expressions list: \n "  ;
    for (Expr ex : argspassed ){
      s += "*Expr : " ; 
      s+= argspassed.toString() ;
      s += "\n" ;
    }
      return s ;          
  }
...
class Variable {
  ...
  public Variable(String name, int uid) {
    this.name = name;
    this.uid  = uid;
